...
const FILESYSTEM_SPACE int = 70000000
const REQUIRED_FREE_SPACE int = 30000000
...
func findMinCorrectSizedFolder(tree map[string]*File) *File {
    neededSpaceToRemove := REQUIRED_FREE_SPACE - (FILESYSTEM_SPACE - tree["/"].size)
    var minFile *File
    minFileSize := REQUIRED_FREE_SPACE

    for _, file := range tree {
        if file.dir {
            if file.size > neededSpaceToRemove && file.size < minFileSize {
                minFile = file
...
func main() {
    ...
    printTree(fileTree)
    minFile := findMinCorrectSizedFolder(fileTree)
    fmt.Printf("Identified %s with size %d\n", minFile.path, minFile.size)
}
